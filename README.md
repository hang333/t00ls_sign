...
其中 `T00LS_USERNAME` 存放你在 Hostloc 的帐户名，`T00LS_PASSWORD` 存放你的帐户密码， `T00LS_MD5` 在存放的密码是 md5 时才需设置， `T00LS_QID` 存放你的登录问题ID，`T00LS_QANS` 存放你的登录问题答案， `SCKEY` 则存放Server酱申请的skey。
